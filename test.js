...
const reviewsObj = {
    totalReviews: document.querySelector('.jFzngs').textContent,
    reviewDate: '',
    reviewNumStars: firstReview.querySelectorAll('.react-stars span').length,
    reviewer: ''
}

// set Total Reviews for Badge
reviewsObj.totalReviews = document.querySelector('.jFzngs').textContent;
// get number of starts in first review
reviewsObj.reviewDate = firstReview.querySelectorAll('.doNdYi')[1].innerHTML;
reviewsObj.reviewNumStars = firstReview.querySelectorAll('.react-stars span').length;
reviewsObj.reviewer = firstReview.querySelectorAll('.MbGlP > .doNdYi + .doNdYi')[1].innerHTML;
...
const reviewBadge = `
        <div class="bhfZaK">
            ...
            <p class="jFzngs">${reviewsObj.totalReviews}</p>
...
const trustAdditionHTML = `
    <div class="trust-addition">
        ...
        <div class="first-review">
            <div class="date">${reviewsObj.reviewDate}</div>
            <div class="num-stars">${reviewsObj.reviewNumStars} Stars</div>
            <div class="stars-imgs">${starsParent.innerHTML}</div>
            <div class="reviewer">${reviewsObj.reviewer}</div>
        </div>
    </div>
`;
